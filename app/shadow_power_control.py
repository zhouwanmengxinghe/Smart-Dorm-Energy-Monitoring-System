"""
shadow_power_control.py — Raspberry Pi device simulator with shadow-driven power cutoff.
...
Usage:
  python shadow_power_control.py
"""
...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        # Subscribe to shadow delta so we receive power_cutoff commands
        ...
        # Fetch the current shadow state on startup
...
def on_message(client, userdata, msg):
    ...
    Two topics are relevant:
      1. shadow/update/delta  → a new desired state has arrived
      2. shadow/get/accepted  → full shadow doc (used to recover state on startup)
    """
    ...
    try:
        ...
        if msg.topic == SHADOW_DELTA_TOPIC:
            ...
            if cutoff_cmd == "true" and not power_cutoff:
                ...
                # No change needed — ack to clear the delta
                print(f"ℹ️  Shadow delta already in sync (power_cutoff={power_cutoff})")
        ...
        # ── Full shadow doc (startup sync) ───────────────────────
            ...
            # Recover version for optimistic locking
            shadow_version = payload.get("version", 0)

            # Check if desired has a power_cutoff command we haven't acted on
        ...
        print(f"⚠️  Non-JSON message on {msg.topic}: {msg.payload.decode()}")
    except Exception as e:
        print(f"❌ Error handling message: {e}")


def on_publish(client, userdata, mid):
    pass  # suppress noisy publish confirmations
...
def publish_sensor_data(client):
    ...
    if power_cutoff:
        # Power is cut — publish zero readings
        voltage = 0.0
        current = 0.0
        power = 0.0
        # cumulative_energy does not increase while power is cut
    else:
        voltage = round(random.uniform(228.0, 232.0), 1)
        current = round(random.uniform(0.5, 4.5), 2)
        power = round(voltage * current, 2)
        # Accumulate energy: power (W) × 30 s / 3 600 000 = kWh increment
...
def main():
    ...
    time.sleep(2)  # give the connection + subscriptions time to settle

    print("\n🚀 Publishing simulated data every 30 s")
